        ...
        <div>
          ...
          <div className="flex flex-col gap-4">
            {Object.keys(apiMap).map(service => (
              ...
              >
                <img src={`https://files.hedera.com/Nav-Icon-${service}-Service-std.svg`} alt={service} className="w-6 h-6" />
                ...
                <table className="table-fixed w-full text-xs">
                  <tbody>
                    {Object.entries(feeResult.details).reduce((rows, [label, detail], index) => {
                      ...
                      rows[rowIndex].push(
                        <td key={label} className="px-3 py-1 whitespace-nowrap border border-gray-700 rounded text-center">
                          {label} (x{detail.value}): {detail.fee.toFixed(5)} USD
                        </td>
                      );
                      return rows;
                    }, []).map((row, i) => (
                      <tr key={i} className="border-none flex justify-start gap-4">
                        {row}
                      </tr>
                    ))}
                  </tbody>
